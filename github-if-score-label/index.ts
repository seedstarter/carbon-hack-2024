...
export const GithubIFScoreLabel = (
  ...
  function setIfOutputYMLPathInfo(
    ...
    const iFOutputYmlPathInfo: IFOutputYmlPathInfo = {
      pathDir: '',
      pathFile: '',
    };
    try {
  ...
  function setDynamicParamsWorkflowYML(workflowObj: WorkflowYMLObj) {
    try {
      const steps = workflowObj['jobs']['update-readme']['steps'];
      if (!steps.length) {
        throw new Error();
      } else {
        steps.map(action => {
          for (const key in action) {
            if (key === 'run' && action['name']?.includes('README')) {
              const updateReadmeAction = action['run'];
